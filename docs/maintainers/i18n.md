<!--
Maintainer documentation: the bilingual (English/Dutch) UI. Covers locale resolution, the
gettext catalogs and the extraction command, the three project-specific extraction keywords,
messages carrying runtime values, locale-dependent number and date formatting, and literal
percent signs. The stack and layout are in development.md.
-->
...
## Messages with runtime values

A display string built with an f-string has a msgid that exists only at runtime, so
`pybabel extract` never records it and the template's `_()` around it matches nothing — the
string renders in English on a Dutch page while every catalog reports 100% translated.
...
A figure is not a msgid and does not go in a catalog, but it still depends
